...
const marcarPagada = db.transaction((id, { terminal, usuario_id } = {}) => {
  ...
  if (r.total_pagar > 0) {
    ...
    const info = db
      .prepare(
        `INSERT INTO caja_movimientos (caja_turno_id, tipo, categoria, tipo_egreso, concepto, monto, forma_pago, referencia_tipo, referencia_id, usuario_id)
         VALUES (?, 'egreso', 'rendicion', ?, ?, ?, 'Efectivo', 'rendicion', ?, ?)`
      )
      .run(turno.id, r.cerrajero_nombre, `${r.cerrajero_nombre} (${formatPeriodoCorto(r.fecha_desde, r.fecha_hasta)})`, r.total_pagar, id, usuario_id || null);
