...
router.post('/', (req, res) => {
  const { nombre, porcentaje_rendicion, porcentaje_urgencia, aporte_fijo, descuento_tarjeta_credito } = req.body;
  if (!nombre || !nombre.trim()) {
    return res.status(400).json({ error: 'El nombre es obligatorio' });
  }
  const info = db
    .prepare(
      `INSERT INTO cerrajeros (nombre, porcentaje_rendicion, porcentaje_urgencia, aporte_fijo, descuento_tarjeta_credito)
       VALUES (@nombre, @porcentaje_rendicion, @porcentaje_urgencia, @aporte_fijo, @descuento_tarjeta_credito)`
    )
    .run({
      nombre: nombre.trim(),
...
router.put('/:id', (req, res) => {
  ...
  const { nombre, porcentaje_rendicion, porcentaje_urgencia, aporte_fijo, descuento_tarjeta_credito, activo } = req.body;
  db.prepare(
    `UPDATE cerrajeros SET
       nombre = @nombre, porcentaje_rendicion = @porcentaje_rendicion, porcentaje_urgencia = @porcentaje_urgencia,
       aporte_fijo = @aporte_fijo, descuento_tarjeta_credito = @descuento_tarjeta_credito, activo = @activo
     WHERE id = @id`
  ).run({
    id,
